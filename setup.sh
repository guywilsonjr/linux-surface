...
if [ "$uselibwacom" = "yes" ]; then
    ...
    urls=$(curl --silent "https://api.github.com/repositories/211578378/releases/latest" \
           | tr ',' '\n' \
...
if [ "$autoinstallkernel" = "yes" ]; then
    ...
    urls=$(curl --silent "https://api.github.com/repositories/155009767/releases/latest" \
           | tr ',' '\n' \
           | grep '"browser_download_url":' \
           | sed -E 's/.*"([^"]+)".*/\1/' \
           | grep '.deb$')
